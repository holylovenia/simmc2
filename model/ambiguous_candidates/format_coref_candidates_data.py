...
Reads SIMMC 2.1 dataset, creates train, devtest, dev formats for coref candidates.
...
def main(args):
    for split in SPLITS:
        ...
        # Result save path.
        if not os.path.exists(args["coref_candidates_save_path"]):
            os.makedirs(args["coref_candidates_save_path"])

        # Reformat into simple strings with positive and negative labels.
        # (dialog string, label)
        coref_candidates_data = []
        for dialog_id, dialog_datum in enumerate(dialogs["dialogue_data"]):
            ...
            for turn_ind, turn_datum in enumerate(dialog_datum["dialogue"]):
                ...
                coref_candidates_data.append(new_datum)
        ...
        print(f"# instances [{split}]: {len(coref_candidates_data)}")
        save_path = os.path.join(
            args["coref_candidates_save_path"],
            f"simmc2.1_coref_candidates_dstc11_{split}.json"
        )
